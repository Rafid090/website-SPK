    ...
    <div class="dashboard-content">
      <section>
        <div class="container py-4 px-5">
          ...
          <?php
            $panggil = mysqli_query($koneksi, "SELECT * from posisi where id_posisi='$id1'");
            while($rank=mysqli_fetch_array($panggil)) {
              echo "<div class='row g-0'> $rank[nama_posisi] </div>";
            }
          ?>
          ...
          <table class="table table-striped">
            <thead>
              <tr>
               ...
               <th class="col-lg-1"> Nilai Akhir </th>
                    </tr>
                  </thead>
                  <tbody>
                  <?php
                      ...
                      $no = 1;
                      $query=mysqli_query($koneksi, "SELECT DISTINCT nama, vektor_v from data_karyawan, nilai, vektor_v where vektor_v.id_karyawan = data_karyawan.id_karyawan and nilai.id_karyawan = data_karyawan.id_karyawan and data_karyawan.tanggal_dataset='$id' and posisi_lamar='$id1' order by vektor_v desc");
                    
                      while($data=mysqli_fetch_array($query)) {
                        echo "<tr><td>$no </td>";
                        echo "<td>$data[nama] </td>"; ?>

                        <td><?php echo round($data['vektor_v'], 3) ?> </td> <?php
                        $no++;
          ...
          <?php
          echo "<a class='btn btn-dark' href='cetakHasil.php?id_dataset=$id&&id_posisi=$id1' role='button' target='_blank'> Cetak Hasil </a>"; ?>
        </div>
    </section
    </div>
  </div>
